...
function generateUser(data) {
	...
	resultsArray.forEach(user => {
		...
		$('#gallery').append($('<div/>').attr("id", firstName + lastName).addClass("card").append($('<div>').addClass("card-img-container").append($('<img>').attr('src', picture).attr('alt', 'profile picture').addClass("card-img"))).append($('<div>').addClass("card-info-container").append($('<h3>').addClass('card-name cap').attr('id', 'name').html(`${firstName} ${lastName}`)).append($('<p>').addClass('card-text').html(`${email}`)).append($('<p>').addClass('card-text cap').html(`${city} ${state}`))))
		// Creating the modals and hiding them for each user
		$('body').append($('<div>').addClass('modal-container').addClass(firstName + lastName).append($('<div>').addClass('modal').append($('<button>').addClass('modal-close-btn').attr('id', 'modal-close-btn').html('<strong>X</strong>').click(function() {
			$('.modal-container').hide()
		})).append($('<div>').addClass('modal-info-container').append($('<img>').attr('src', picture).attr('alt', 'profile picture').addClass("modal-img")).append($('<h3>').addClass('modal-name').attr('id', 'name').html(`${firstName} ${lastName}`)).append($('<p>').addClass('modal-text').html(`${email}`)).append($('<p>').addClass('modal-text cap').html(`${city}`)).append($('<hr>')).append($('<p>').addClass('modal-text').html(`${phone}`)).append($('<p>').addClass('modal-text').html(`${streetAddress}`)).append($('<p>').addClass('modal-text').html(`Birthday: ${birthMonth}-${birthDay}-${birthYear}`)).append($('<button>').text('Previous').addClass('myButton').click(function(e) {
			showPrevious(e)
		})).append($('<button>').addClass('myButton').text('Next').click(function(e) {
			showNext(e)
		})))).hide())
	})
}
...
// This function dynamically adds the search bar to the top of the page
...
// Checks to see if the search input matches any of the names and shows the matches
...
// Shows the next modal after the currently shown modal
function showNext(e) {
...
// Shows the previous modal after the currently shown modal
